...
function FreelancerWorks() {
  ...
  const hasMatchingOrders = Array.isArray(ordersData) && ordersData.some(order => ["Work Started", "Completed", "Payment Pending"].includes(order.status));
              ...
              ordersData.map((order) => (
                <div className="flex-1 bg-gray-200 rounded-lg shadow-xl mt-4 p-6" key={order.id}>
                  <div className="relative px-4 mt-4">
                    {order.status === "Work Started" || order.status === "Completed" || order.status === "Payment Pending" ? (
